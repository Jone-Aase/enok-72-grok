...
const NORGE_SURFACE_CONTROL_POINTS = [
  { id: 'selsoy', name: 'Selsøy gården / polarsirkelen', lat: 66.5502, lon: 12.8462 },
  { id: 'kveitanosen', name: 'Kveitanosen / polarsirkelen', lat: 66.55, lon: 12.6383 },
  { id: 'arctic-center', name: 'Arctic Circle Center', lat: 66.55, lon: 15.3266 },
];
const NORGE_SURFACE_DETAIL = {
  ...
  maxZoom: 18,
  tileSize: 256,
  bounds: { latMin: 57.7, latMax: 71.5, lonMin: 3.8, lonMax: 32.2 },
};
...
function currentNorgeDetailSources() {
  const selectedBase = document.querySelector('input[name="norge-base"]:checked')?.value || 'topograatone';
  const sources = [];
...
function updateNorgeDetailTiles() {
  ...
  if (!detailLayer || !norgeSurface) return;
  ...
  tileRange = fitTileRangeToBudget({ xMin, xMax, yMin, yMax, count }, sources.length);
  if (!tileRange) return;
  ...
  for (const source of sources) {
    for (let x = tileRange.xMin; x <= tileRange.xMax; x++) {
      for (let y = tileRange.yMin; y <= tileRange.yMax; y++) {
        ...
        if (b.east < NORGE_SURFACE_DETAIL.bounds.lonMin || b.west > NORGE_SURFACE_DETAIL.bounds.lonMax ||
            b.north < NORGE_SURFACE_DETAIL.bounds.latMin || b.south > NORGE_SURFACE_DETAIL.bounds.latMax) {
          continue;
        }
        const nw = aeProject(b.north, b.west);
        const ne = aeProject(b.north, b.east);
        const sw = aeProject(b.south, b.west);
        const se = aeProject(b.south, b.east);
        const corners = [
          worldToCanvasPoint(nw.x, nw.z),
          worldToCanvasPoint(ne.x, ne.z),
          worldToCanvasPoint(sw.x, sw.z),
          worldToCanvasPoint(se.x, se.z),
        ];
        ...
        if (right < 0 || left > cw || bottom < 0 || top > ch) continue;
        ...
        img.style.left = '0px';
        img.style.top = '0px';
        img.style.width = `${NORGE_SURFACE_DETAIL.tileSize}px`;
        img.style.height = `${NORGE_SURFACE_DETAIL.tileSize}px`;
        img.style.transform = `translate(${left.toFixed(2)}px, ${top.toFixed(2)}px) scale(${(tileW / NORGE_SURFACE_DETAIL.tileSize).toFixed(8)}, ${(tileH / NORGE_SURFACE_DETAIL.tileSize).toFixed(8)})`;
...
function addNorgeSurfaceControlPoints(overlay) {
  ...
  const arcPoints = [];
  for (let lon = 11.5; lon <= 15.7; lon += 0.1) {
    arcPoints.push(projectNorgeSurfacePoint(66.55, lon));
  }
  group.appendChild(svgEl('path', {
    d: norgeSurfacePath(arcPoints),
    class: 'norge-surface-line',
    stroke: '#ffef6e',
    'stroke-width': 3,
    'stroke-opacity': '0.9',
...
function renderNorgeSurfaceLayers() {
  ...
  if (document.getElementById('norge-layer-geo-grid')?.checked) addNorgeSurfaceGrid(overlay, false);
  if (document.getElementById('norge-layer-square-grid')?.checked) addNorgeSurfaceGrid(overlay, true);
  if (document.getElementById('norge-layer-cities')?.checked) addNorgeSurfaceCities(overlay);
...
function updateNorgeCalibrationStatus() {
  ...
  NORGE_SURFACE_CONTROL_POINTS.forEach(point => {
    ...
    const targetWorld = aeProject(point.lat, point.lon);
...
function bindNorgeSurfaceEngine() {
  ...
  ['norge-layer-geo-grid', 'norge-layer-square-grid', 'norge-layer-cities'].forEach(id => {
    document.getElementById(id)?.addEventListener('change', renderNorgeSurfaceLayers);
  });
...
function initNorgeSurfaceMap() {
  ...
  document.querySelectorAll('input[name="norge-base"], #norge-layer-sjokart, #norge-layer-nib')
    .forEach(el => {
      el.disabled = false;
      el.title = '';
      el.addEventListener('change', updateNorgeSurfaceRasters);
    });
